...
function createDownloadToken(url, filename, now = Date.now()) {
  purgeExpired(now);
  const token = randomUUID();
  records.set(token, { url, filename, expiresAt: now + TOKEN_TTL_MS });
  return token;
}
function consumeDownloadToken(token, now = Date.now()) {
  purgeExpired(now);
...
function isAllowedDownloadRequest(clientKey, now = Date.now()) {
  const recent = (attempts.get(clientKey) ?? []).filter((timestamp2) => now - timestamp2 < RATE_LIMIT_WINDOW_MS);
...
function detectPlatform(value) {
  try {
    ...
    return Object.entries(PLATFORM_HOSTS).find(([, pattern]) => pattern.test(host))?.[0] ?? null;
  } catch {
    return null;
  }
}
function valueOf(data, ...keys) {
  for (const key of keys) if (data[key] !== void 0 && data[key] !== null && data[key] !== "") return data[key];
  return void 0;
}
function normalizeMetadata(platform, originalUrl, payload) {
  ...
  return {
    ...
    qualities: Array.isArray(data.qualities) ? data.qualities.filter((item) => typeof item === "string") : typeof data.quality === "string" ? [data.quality] : [],
    canDownload: platform !== "facebook"
  };
}
async function socialFetch(url, init) {
  const retryableStatuses = /* @__PURE__ */ new Set([429, 502, 503, 504]);
  let lastError = null;
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    try {
      ...
      if (payload && typeof payload === "object" && payload.success === false) {
        ...
        const message = code === "insufficient_credits" ? "The download provider has no credits available. Please try again later." : String(payload.message || "The provider could not process this video.");
...
var inputShape = z2.object({ url: z2.string().trim().url().max(2048) });
var downloadInput = inputShape.extend({ format: z2.enum(["mp4", "mp3", "avi", "webm", "m4a", "ogg", "wav"]).default("mp4"), quality: z2.enum(["240p", "360p", "480p", "720p", "1080p"]).optional() });
var MAX_DOCUMENTED_VIDEO_QUALITY = "1080p";
var VIDEO_FORMATS = /* @__PURE__ */ new Set(["mp4", "avi", "webm"]);
var appRouter = router({
  ...
  downloader: router({
    ...
    download: publicProcedure.input(downloadInput).mutation(async ({ input, ctx }) => {
      ...
      const requestedQuality = input.quality ?? (VIDEO_FORMATS.has(input.format) ? MAX_DOCUMENTED_VIDEO_QUALITY : void 0);
